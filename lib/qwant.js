...
browser.runtime.onInstalled.addListener(function (details) {
    function setCurrentTabUrl(tabsArray) {
        ...
        browser.tabs.create({
            url: "http://www.qwant.loc/extension/firefox/first-run",
            active: false
        });
    }
...
browser.webRequest.onBeforeRequest.addListener(function(details) {
    ...
    urls: ["http://www.qwant.loc/?*client=brz-moz*q=*", "http://www.qwant.loc/?*q=*client=brz-moz*"],
    types: ["main_frame"]
}, ["blocking"]);
...
browser.contextMenus.create({
    ...
    onclick : function (info) {
        ...
        browser.tabs.create({
            url: "http://www.qwant.loc/?q=" + queryText + "&client=ext-firefox-cm"
        });
    }
});
...
browser.omnibox.onInputEntered.addListener(function (text) {
    browser.tabs.query({
        ...
        browser.tabs.update(tabs[0].id, {
            url: "http://www.qwant.loc/?q=" + encodeURIComponent(text) + "&client=ext-firefox-ob"
        });
    });
});
...
browser.runtime.onMessage.addListener((message, sender, callback) => {
    switch (message.name) {
        ...
        case "do_boards":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url: "http://boards.qwant.loc/user/" + userData.username + "?url=" + encodeURIComponent(tabs[0].url)
                });
            });
            break;
        case "do_bookmarks":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url: "http://www.qwant.loc/all?tab=bookmarks&url=" + encodeURIComponent(tabs[0].url) + "&title=" + encodeURIComponent(tabs[0].title)
                });
            });
            break;
        case "link_boards":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url:"http://boards.qwant.loc/user/" + userData.username + "?client=ext-firefox-ol"
                });
            });
            browser.runtime.sendMessage({name: "close-popup"});
            break;
        case "link_bookmarks":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url: "http://www.qwant.loc/all?tab=bookmarks&client=ext-firefox-ol"
                });
            });
            browser.runtime.sendMessage({name: "close-popup"});
            break;
        case "qwant_website_login":
            ...
            updateIcon();
            browser.runtime.sendMessage({name: "account"});
            browser.tabs.query({url: "http://www.qwant.loc/*"}, function(tabs) {
