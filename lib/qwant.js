...
browser.runtime.onInstalled.addListener(function (details) {
    function setCurrentTabUrl(tabsArray) {
        ...
        browser.tabs.create({
            url: "https://www.qwant.loc/extension/firefox/first-run",
            active: false
        });
    }
...
browser.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
    if (changeInfo.url && changeInfo.url.match(/^https:\/\/(www|boards)\.qwant.loc/)) {
        browser.tabs.executeScript({
...
browser.webRequest.onBeforeRequest.addListener(function(details) {
    ...
    urls: ["https://www.qwant.loc/?*client=brz-moz*q=*", "https://www.qwant.loc/?*q=*client=brz-moz*"],
    types: ["main_frame"]
}, ["blocking"]);

browser.runtime.setUninstallURL('https://www.qwant.loc/extension/feedback?v=' + manifest.version);
...
browser.contextMenus.create({
    ...
    onclick : function (info) {
        ...
        browser.tabs.create({
            url: "https://www.qwant.loc/?q=" + queryText + "&client=ext-firefox-cm"
        });
    }
});
...
browser.omnibox.onInputEntered.addListener(function (text) {
    browser.tabs.query({
        ...
        browser.tabs.update(tabs[0].id, {
            url: "https://www.qwant.loc/?q=" + encodeURIComponent(text) + "&client=ext-firefox-ob"
        });
    });
});
...
browser.runtime.onMessage.addListener((message, sender, callback) => {
    switch (message.name) {
        ...
        case "do_boards":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url: "https://boards.qwant.loc/user/" + userData.username + "?url=" + encodeURIComponent(tabs[0].url)
                });
            });
            break;
        case "do_bookmarks":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url: "https://www.qwant.loc/all?tab=bookmarks&url=" + encodeURIComponent(tabs[0].url) + "&title=" + encodeURIComponent(tabs[0].title)
                });
            });
            break;
        case "link_boards":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url:"https://boards.qwant.loc/user/" + userData.username + "?client=ext-firefox-ol"
                });
            });
            browser.runtime.sendMessage({name: "close-popup"});
            break;
        case "link_bookmarks":
            ...
            browser.tabs.query({active: true, currentWindow: true}, function(tabs) {
                browser.tabs.create({
                    url: "https://www.qwant.loc/all?tab=bookmarks&client=ext-firefox-ol"
                });
            });
            browser.runtime.sendMessage({name: "close-popup"});
            break;
        case "qwant_website_login":
            ...
            updateIcon();
            browser.runtime.sendMessage({name: "account"});
            browser.tabs.query({url: "https://www.qwant.loc/*"}, function(tabs) {
