...
chrome.runtime.onInstalled.addListener(function (details) {
    function setCurrentTabUrl(tabsArray) {
        ...
        chrome.tabs.create({
            url: "https://org.qwant.com/extension/thanks",
            active: false
        });
    }
...
chrome.runtime.setUninstallURL('https://org.qwant.com/extension/feedback?v=' + manifest.version);
...
chrome.contextMenus.create({
    ...
    onclick : function (info) {
        ...
        chrome.tabs.create({
            url: "https://org.qwant.com/?q=" + queryText + "&client=ext-chrome-cm"
        });
    }
});
...
chrome.omnibox.onInputEntered.addListener(function (text) {
    chrome.tabs.query({
        ...
        chrome.tabs.update(tabs[0].id, {
            url: "https://org.qwant.com/?q=" + encodeURIComponent(text) + "&client=ext-chrome-ob"
        });
    });
});
...
var updateIcon = () => {
    privacy.isEnabled(isPrivacyEnabled => {
        if(userIsLogged()) {
            if(isPrivacyEnabled) {
                chrome.browserAction.setIcon({ path: '../img/picto/q4-48.png' });
            } else {
                chrome.browserAction.setIcon({ path: '../img/picto/q3-48.png' });
            }
        } else {
            if(isPrivacyEnabled) {
                chrome.browserAction.setIcon({ path: '../img/picto/q2-48.png' });
            } else {
                chrome.browserAction.setIcon({ path: '../img/picto/q1-48.png' });
            }
        }
    });
};
...
chrome.runtime.onMessage.addListener((message, sender, callback) => {
    ...
    switch (message.name) {
        ...
        case "do_bookmarks":
            ...
            chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
                chrome.tabs.create({
                    url: "https://org.qwant.com/all?tab=bookmarks&url=" + encodeURIComponent(tabs[0].url) + "&title=" + encodeURIComponent(tabs[0].title)
                });
            });
            break;
        case "link_boards":
        ...
        case "link_bookmarks":
            ...
            chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
                chrome.tabs.create({
                    url: "https://org.qwant.com/all?tab=bookmarks&client=ext-chrome-ol"
                });
            });
            chrome.runtime.sendMessage({name: "close-popup"});
            break;
        case "qwant_website_login":
            ...
            updateIcon();
            chrome.runtime.sendMessage({name: "account"});
            chrome.tabs.query({url: "https://org.qwant.com/*"}, function(tabs) {
